    ...
    Object.keys(kolonyAppInfo.dependencies).forEach(function(moduleName){
        if (moduleName.indexOf("kolony-")>-1){
            self.availableModules[moduleName] = {
                ...
                construct: require(path.join(__dirname, '../../'+moduleName+'/'+moduleName+'.js'))
            }
        }
    });
}
...
Worker.prototype.start = function(){
    ...
    if(self.app.kolony.db) {
        self.db = kolony.orm(self.app.kolony.db);
    }
    ...
    if (self.config.modules){
        self.config.modules.forEach(function(moduleName){
            ...
            if (module===undefined){
                // Check if the /kolonies/{current-kolony}/{current-container}.json file has all the modules which are in the package.json
                // Example: current-kolony = development, current-container = main
                throw new Error(moduleName+" is not available");
                return;
            }
            var alias = moduleName.replace("kolony-module-", "");

            var moduleInstance = module.construct({name: alias, package: moduleName}, self);
            moduleInstance.aliases.forEach(function(alias){
                self.modules[alias] = moduleInstance;
            })
