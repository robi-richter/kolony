...
function Module(config, worker){
    ...
    if (undefined !== config.alias){
        if (config.alias instanceof Array){
            self.aliases = self.aliases.concat(config.alias);
        }

        if ('string' == typeof config.alias){
            self.aliases.push(config.alias);
        }
    }
...
Module.prototype.baseInit = function(){
    ...
    if (fs.existsSync(modelsPath)){
        fs.readdirSync(modelsPath).forEach(function(file) {
            ...
            self.models[modelName] = require(modelsPath+"/"+file)(self.worker.db, self);
        });
    }
